...
form.addEventListener("submit", function (evento) {
  ...
  if (imc > 40) {
    return setResultado(
      `Seu IMC é de ${imc}, classificado como: Obesidade Grau III`,
      "Refluxo, dificuldade para se mover, escaras, diabetes, infarto, AVC",
      "red"
    );
  } else if (imc >= 35) {
    return setResultado(
      `Seu IMC é de ${imc}, classificado como: Obesidade Grau II`,
      "Apneia do sono, falta de ar",
      "orange2"
    );
  } else if (imc >= 30) {
    return setResultado(
      `Seu IMC é de ${imc}, classificado como: Obesidade Grau I`,
      "Diabetes, angina, infarto, aterosclerose",
      "orange1"
    );
  } else if (imc >= 25) {
    return setResultado(
      `Seu IMC é de ${imc}, classificado como: Acima do peso`,
      "Fadiga, má circulação, varizes",
      "yellow"
    );
  } else if (imc >= 18.5) {
    return setResultado(
      `Seu IMC é de ${imc}, classificado como: Peso normal`,
      "Menor risco de doenças cardíacas e vasculares",
      "green"
    );
  } else if (imc >= 17) {
    return setResultado(
      `Seu IMC é de ${imc}, classificado como: Abaixo do peso`,
      "Fadiga, stress, ansiedade",
      "yellow"
    );
  } else {
    return setResultado(
      `Seu IMC é de ${imc}, classificado como: Muito abaixo do peso`,
...
function setResultado(result, podeAcontecer, classP) {
  ...
  resultado.innerHTML += "<h4>Possíveis consequências:</h4>";
